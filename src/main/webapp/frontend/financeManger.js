async function getAllTicketsByStatus(event){
    ...
    let url = "http://localhost:8080/ers_mantesh/manager/"
...
function createReimbursementRow(reimbursement, parentElement){
    ...
    $${reimbursement.amount} for ${reimbursement.type.toLowerCase()} requested by ${reimbursement.firstName.charAt(0).toUpperCase()}${reimbursement.firstName.slice(1).toLowerCase()} ${reimbursement.lastName.charAt(0).toUpperCase()}${reimbursement.lastName.slice(1).toLowerCase()}`
...
function addButtons(card){
    ...
    approveBut.addEventListener("click", async (event) =>{
        ...
        let res = await fetch("http://localhost:8080/ers_mantesh/manager/approvereq", {
            method:"POST",
    ...
    declineBut.addEventListener("click", async (event) =>{
        ...
        let res = await fetch("http://localhost:8080/ers_mantesh/manager/denyreq", {
            method:"POST",
            body: `reimbursementid=${event.target.parentElement.parentElement.dataset.id}`,
            headers:{
                "Content-Type" : "application/json"
...
async function authenticateUser() {
    try {
      let res = await fetch("http://localhost:8080/ers_mantesh/login/financemanager", {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
